...
const filename = '037_service_type_pricing.sql';
  ...
  const verify = await client.query(`
    SELECT
      EXISTS (
        ...
        WHERE table_name = 'pcmazing_service_types' AND column_name = 'labor_price'
      ) AS has_labor_price,
      EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'pcmazing_service_types' AND column_name = 'cost_price'
      ) AS has_cost_price
  `);
