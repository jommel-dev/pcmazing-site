...
const filename = '042_printing.sql';
  ...
  const verify = await client.query(`
    SELECT
      EXISTS (
        ...
        WHERE table_schema = 'public' AND table_name = 'pcmazing_printing_settings'
      ) AS has_settings,
      EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'pcmazing_printing_templates'
      ) AS has_templates
  `);
