...
const filename = '020_service_customer_and_custom_parts.sql';
    ...
    SELECT
      EXISTS (
        ...
        WHERE table_schema = 'public'
          AND table_name = 'pcmazing_services'
          AND column_name = 'customer_name'
      ) AS has_customer_name,
      EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_schema = 'public'
          AND table_name = 'pcmazing_service_parts'
          AND column_name = 'custom_item_name'
      ) AS has_custom_item_name,
      EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_schema = 'public'
