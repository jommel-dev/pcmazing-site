...
const UPLOAD_SUBDIR = 'company-expense-files';
...
export async function saveCompanyExpenseAttachmentFile(
  expenseId: number,
  file: Express.Multer.File,
): Promise<{ fileUrl: string; kind: 'receipt' | 'file' }> {
  if (!file) {
    throw new BadRequestException('Attachment file is required.');
  }

  if (!FILE_MIME.has(file.mimetype)) {
    throw new BadRequestException('Unsupported file type. Use images (JPEG, PNG, WebP, GIF) or PDF.');
  }
  ...
  const filename = `expense-${expenseId}-${Date.now()}${extension}`;
  await writeFile(join(uploadDir, filename), file.buffer);

  return {
    fileUrl: toPublicUploadUrl(UPLOAD_SUBDIR, filename),
    kind: IMAGE_MIME.has(file.mimetype) ? 'receipt' : 'file',
  };
}

export async function deleteCompanyExpenseAttachmentFile(
